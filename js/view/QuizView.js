...
export class QuizView {
    ...
    showQuestion(currentQuestion) {
        ...
        question.innerHTML =
            "Vraag " + currentQuestion.getQuestionID() + ".<br>" +
            "Wat is " + currentQuestion.getQuestion() + "?"
        ;
        const options=currentQuestion.getOptions();
        this.getElement('#option1').innerHTML = options[0];
        this.getElement('#option2').innerHTML = options[1];
        this.getElement('#option3').innerHTML = options[2];
        this.getElement('#option4').innerHTML = options[3];
    ...
    showNav(currentQuestion, questions) {
        ...
        questions.forEach(vraag => {
            if (vraag.getQuestionID() === currentQuestion.getQuestionID()) {
                this.getElement('#no' + currentQuestion.getQuestionID()).style['border'] = '3px solid ghostwhite';
            }
            else {
                this.getElement('#no' + vraag.getQuestionID()).style['border'] = '5px solid  lightgray';
            }
        });

        questions.forEach(vraag => {
            if (vraag.getGivenAnswer() !== -1) {
                this.getElement('#no' + vraag.getQuestionID()).style['background-color'] = 'lightblue';
            }
            else {
